#!/usr/bin/python3 
 ...
 try: 
   print (f"{p}[{y}!{p}] {r}Connecting to Server") 
     ...
     if echa == 'al': 
       ...
       while True: 
         try: 
           ...
           show = input(f"{p}[{y}?{p}] {w}Download [{g}Y/{w}{r}n{w}] {P}").lower() == 'y' 
           if show: 
 ...
 def menu(): 
   ...
   if choice == '1' or choice == '01': 
     ...
     os.system('clear') 
     print (f"{logo}\n{g}INFO SCRIPT\n========================\n{p}[{y}✓{p}] {c}Author: {g}Mr Juice\n{p}[{y}✓{p}] {c}Team  : {colors()}TNT {colors()}ANONYMOUS {r}ZIM{w}BABWE\n{p}[{y}✓{p}] {c}Script: {colors()}{os.path.basename(sys.argv[0])}\n{p}[{y}✓{p}] {c}Path  : {os.path.realpath(sys.argv[0])}\n{p}[{y}✓{p}] {c}Size  : {os.stat(os.path.realpath(sys.argv[0])).st_size} Byte\n{p}[{y}✓{p}] {c}Link  : {colors()}https://github.com/mrjuice01/Virtex\n{p}[{y}✓{p}] {c}Update: {colors()}{UPDATE}\n{p}[{y}✓{p}] {c}Versi : 1.1\n\n{g}Contact Me ^_^\n==================\n{p}[{y}✓{p}] {c}Github: {colors()}https://github.com/mrjuice01/\n{p}[{y}✓{p}] {c}Fb.   : {colors()}https://www.instagram.com/mr_juice7\n{p}[{y}✓{p}] {c}Wa.   : {colors()}+2 6 3 7 8 0 6 9 9 9 8 8\n{p}[{y}✓{p}] {c}Email : {colors()}mrjuice01@gmail.com\n{a}") 
